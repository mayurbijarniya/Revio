...
4.  Technical Debt Accumulation: Incremental changes that break global architectural patterns because the reviewer lacks context.
...
Revio is designed as a modular, event-driven system. Below is a high-level representation of the RAG (Retrieval-Augmented Generation) pipeline:
...
3.  **Review Orchestrator**:
    ...
    - Reasoning Loop: Feeds the diff + retrieved context into a high-reasoning LLM with a specialized system prompt.
...
Teams can define specific "rules" in the dashboard. These rules are injected into the AI's reasoning loop, allowing for repo-specific enforcement of architecture (e.g., "Always use the Repository pattern for database access").
...
2.  **Semantic Retrieval**: It queries the Qdrant vector store to find the top relevant code snippets across the entire repository.
...
1.  **Background Workers**: For high-volume repositories, Revio uses BullMQ and Redis to manage indexing and review jobs outside of the request-response cycle.
2.  **Serverless Optimization**: On platforms like Vercel, Revio uses the Next.js `after()` API to ensure long-running AI tasks complete even after the HTTP response is sent.
